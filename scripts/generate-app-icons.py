...
with tempfile.TemporaryDirectory(prefix="app-icons-", dir=output) as temporary:
    ...
    for size in sizes:
        ...
        subprocess.run(
            ["optipng", "-strip", "all", "-o7", "-quiet", str(png)], check=True
        )
    ...
    (staging / "icon.icns").write_bytes(
        struct.pack(">4sI", b"icns", 8 + len(chunks)) + chunks
    )
    ...
    for size in ico_sizes:
        ...
        directory.extend(
            struct.pack(
                "<BBBBHHII", size % 256, size % 256, 0, 0, 1, 32, length, offset
            )
        )
        offset += length
    (staging / "icon.ico").write_bytes(
        directory + b"".join(images[size] for size in ico_sizes)
    )
