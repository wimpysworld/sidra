...
export function initServiceSwitch(deps: {
 getTray: () => Tray | null;
...
export function switchService(id: MusicServiceId, targetUrl?: string): void {
 ...
 // The loadURLCallback() navigation replaces the document, so the next injection
 // must not call removeInsertedCSS() with a key from the old document.
 notifyDocumentReplacing();
 // Resolve the default after setMusicService() so it reads the persisted service.
