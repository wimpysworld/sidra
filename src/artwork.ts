...
async function fetchArtwork(url: string): Promise<string | null> {
  ...
  try {
    ...
    // net.fetch types its body as the DOM ReadableStream, while Readable.fromWeb
    // takes the stream/web type. The reader's ArrayBuffer variance requires the cast.
    const body =
