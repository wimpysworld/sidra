...
export function createLinuxNotifications(
  ...
  const dispose = async (): Promise<void> => {
    ...
    (connection as unknown as EventEmitter).removeListener(
      "message",
      onMessage,
    );
  ...
  const onMessage = (message: Message): void => {
    ...
    if (
      message.sender === BUS_NAME &&
      message.interface === BUS_NAME &&
      message.path === BUS_PATH &&
  ...
  const ready = Promise.resolve().then(async () => {
    try {
      ...
      try {
        ...
        owner =
          typeof currentOwner === "string" && currentOwner.startsWith(":")
            ? currentOwner
            : "";
        reportOwner(owner !== "");
      } catch {
  ...
  const deliver = async (
    ...
    if (
      ...
      (generation !== refreshGeneration || !notificationId)
    )
      return;
    ...
    try {
      const reply = await call(BUS_NAME, "GetNameOwner", "s", [NAME]);
      const currentOwner: unknown = reply?.body[0];
      if (typeof currentOwner !== "string" || !currentOwner.startsWith(":"))
        return;
      if (
        !operation.active ||
        !bus ||
      ...
      const actions =
        ...
        capabilities.body[0].includes("actions");
      ...
      const result = await call(owner, "Notify", "susssasa{sv}i", [
        ...
        actions
          ? [
              "default",
              "",
              notification.playbackAction,
              notification.playbackLabel,
              "previous",
  ...
  const show = (
    ...
    pending = pending.then(async () => {
      ...
      try {
        await Promise.race([
          deliver(notification, isCurrent, operation, refreshGeneration),
          cancelled,
        ]);
      } finally {
        operation.active = false;
