...
const linuxAdapter = vi.hoisted(() => ({
  show: vi.fn().mockResolvedValue(undefined),
  dispose: vi.fn(),
}));
vi.mock("../src/linuxNotifications", () => ({
  createLinuxNotifications: () => linuxAdapter,
}));
...
describe("notifications integration", () => {
  ...
  it.each(["win32", "darwin"])(
    "dispatches only the two native actions on %s",
    async (platform) => {
  ...
  it.each(["win32", "darwin"])(
    "replaces the previous native notification on %s",
    async (platform) => {
      setPlatform(platform);
      player.emitNowPlaying(TRACK);
      await vi.advanceTimersByTimeAsync(DEBOUNCE_MS);
      ...
      expect(notifyFake.built[1].options).toMatchObject({
        id: expect.any(String),
        groupId: "playback",
      });
  ...
  it.each(["win32", "darwin"])(
    "closes only a retired notification when it displays late on %s",
    async (platform) => {
      setPlatform(platform);
      player.emitNowPlaying(TRACK);
      await vi.advanceTimersByTimeAsync(DEBOUNCE_MS);
  ...
  it("removes only the playback group on macOS startup and quit, even while disabled", () => {
    ...
    init({ player, getMainWindow: () => null });
    expect(Notification.removeGroup).toHaveBeenCalledExactlyOnceWith(
      "playback",
    );
    expect(createNotification).not.toHaveBeenCalled();
    quit();
    expect(vi.mocked(Notification.removeGroup).mock.calls).toEqual([
      ["playback"],
      ["playback"],
    ]);
  });
  ...
  it("continues delivery when macOS history cleanup fails", async () => {
    quit();
    setPlatform("darwin");
    vi.mocked(Notification.removeGroup).mockImplementationOnce(() => {
  ...
  it("uses translated labels for native buttons", async () => {
  ...
  it("uses only the Linux adapter and disposes it on quit", async () => {
    ...
    expect(notification).toMatchObject({
      title: TRACK.name,
      previous: ACTION_LABELS.previous,
      next: ACTION_LABELS.next,
      playbackAction: "play",
      playbackLabel: ACTION_LABELS.play,
    });
  ...
  it.each([undefined, 3600000])(
    "coalesces the station and first radio song with duration %s",
    async (durationInMillis) => {
      ...
      expect(shown()?.options).toMatchObject({
        title: "Radio Song",
        body: "Radio Artist",
        icon: "/tmp/sidra-test/artwork.jpg",
      });
      expect(downloadArtwork).toHaveBeenCalledExactlyOnceWith(
        STATION.artworkUrl,
      );
    },
  );
  ...
  it("ignores duplicate timed display fields and catalogue-only enrichment without delaying delivery", async () => {
    ...
    player.emitTimedMetadata({
      ...RADIO_SONG,
      playParams: { kind: "song", catalogId: "catalogue-id" },
    });
    await vi.advanceTimersByTimeAsync(DEBOUNCE_MS);
    expect(notifyFake.built).toHaveLength(1);
  });
  ...
  it.each(["win32", "darwin"])(
    "uses an explicit primary action and checks the latest snapshot on %s",
    async (platform) => {
      setPlatform(platform);
      const send = vi.fn();
      initCommandBridge(send);
      ...
      await vi.advanceTimersByTimeAsync(DEBOUNCE_MS);
      const notification = shown()!;
      expect(notification.options.actions?.[0].text).toBe(
        getTrayStrings().pause,
      );
  ...
  it.each(["win32", "darwin", "linux"])(
    "keeps explicit actions tied to stable playback during transient states on %s",
    async (platform) => {
      setPlatform(platform);
      const send = vi.fn();
      initCommandBridge(send);
  ...
  it("refreshes a pending display once after it settles without a show loop", async () => {
    ...
    shown()!.handlers.show();
    expect(notifyFake.built).toHaveLength(2);
    expect(notifyFake.built[1].options.actions?.[0].text).toBe(
      getTrayStrings().play,
    );
  ...
  it("refreshes Linux buttons without artwork work and makes stale explicit actions harmless", async () => {
    ...
    expect(notification).toMatchObject({
      ...
      playbackLabel: ACTION_LABELS.pause,
    });
    player.emitPlaybackState(PlaybackState.Paused);
    expect(linuxAdapter.show.mock.calls[1][0]).toMatchObject({
      playbackAction: "play",
      playbackLabel: ACTION_LABELS.play,
    });
