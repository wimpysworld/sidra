...
const realEmitPropertiesChanged =
...
let win: WindowStub;
// Keep winContents separate because win.webContents throws after destruction, while teardown assertions still need direct access to its spies.
...
beforeEach(() => {
  ...
  vi.spyOn(
    ...
    "emitPropertiesChanged",
  ).mockImplementation((iface, changed, invalidated) => {
...
describe("MPRIS fullscreen", () => {
  it("exports boolean fullscreen properties with the required access", () => {
    ...
    expect(root.$introspect().property).toEqual(
  ...
  it.each([true, false])(
    "waits for the window transition after requesting %s",
    (value) => {
      ...
      expect(
        dbus.interface.Interface.emitPropertiesChanged,
      ).toHaveBeenCalledWith(root, { Fullscreen: value });
    },
  );
  ...
  it.each(["enter-full-screen", "leave-full-screen"])(
    "logs an emission failure on %s without throwing",
    (event) => {
      initInterfaces();
      vi.mocked(
        dbus.interface.Interface.emitPropertiesChanged,
  ...
  it("removes both listeners before disconnecting the bus", () => {
    ...
    busStub.disconnect.mockImplementationOnce(() => {
      listenersAtDisconnect.push(
        win.listenerCount("enter-full-screen"),
...
const COMMAND_CASES: ReadonlyArray<{
  ...
  },
  {
    ...
    channel: "player:next",
    args: [],
    invoke: (iface) => iface.Next(),
  },
  {
    method: "Previous",
    channel: "player:previous",
    args: [],
    invoke: (iface) => iface.Previous(),
  },
  {
    method: "Pause",
    channel: "player:pause",
    args: [],
    invoke: (iface) => iface.Pause(),
  },
  {
    method: "PlayPause",
    channel: "player:playPause",
    args: [],
    invoke: (iface) => iface.PlayPause(),
  },
  {
    method: "Stop",
    channel: "player:stop",
    args: [1],
    invoke: (iface) => iface.Stop(),
  },
  {
    method: "Play",
    channel: "player:play",
    args: [],
    invoke: (iface) => iface.Play(),
  },
  {
    method: "Seek",
    channel: "player:seek",
    args: [2.5],
    invoke: (iface) => iface.Seek(2_500_000n),
  },
  {
    method: "SetPosition",
    channel: "player:seek",
...
describe("MPRIS command provenance", () => {
  it.each(COMMAND_CASES)(
    ...
    ({ method, channel, args, invoke }) => {
      ...
      const commandLog =
        method === "Volume"
          ? log.scope("mpris").debug
          : log.scope("mpris").info;
  ...
  it.each(COMMAND_CASES)(
    "logs $method as dropped when no window exists",
  ...
  it("logs method-only provenance for Raise and Quit", () => {
    ...
    expect(log.scope("mpris").info).toHaveBeenCalledWith(
      "source=mpris method=Raise result=sent",
    );
    expect(log.scope("mpris").info).toHaveBeenCalledWith(
      "source=mpris method=Quit result=sent",
    );
  });

  it.each(COMMAND_CASES)(
    "drops $method once the window is destroyed",
    ({ method, channel, invoke }) => {
      vi.useFakeTimers();
      const iface = initPlayerInterface();
      win.isDestroyed.mockReturnValue(true);
  ...
  it("logs a sent Volume burst at debug without per-value info records", () => {
    ...
    const provenance =
      "source=mpris method=Volume channel=player:setVolume result=sent";
  ...
  it("does not log accepted or rejected SetPosition request data", () => {
    ...
    expect(mprisLogText()).toContain(
      "source=mpris method=SetPosition channel=player:seek result=sent",
    );
...
describe("MPRIS teardown", () => {
  ...
  // Teardown must not read webContents because window destruction precedes will-quit and makes the native getter throw.
  // disconnectBus() runs last, so its call proves that the complete handler ran.
...
describe("MPRIS Stop", () => {
  ...
  it.each(["item", "document", "playing"])(
    "ignores stale completion after %s changes",
    (change) => {
      const iface = initPlayerInterface();
      iface.Stop();
  ...
  it("clears the completion timer and listener on quit", () => {
    ...
    expect(log.scope("mpris").warn).not.toHaveBeenCalledWith(
      "Stop completion timed out",
    );
  });
});
...
describe("MPRIS playback capabilities", () => {
  it("starts with the cached snapshot and clears capabilities on navigation", () => {
    ...
    const iface = initPlayerInterface();
    expect([iface.CanPlay, iface.CanPause, iface.CanSeek]).toEqual([
      true,
      true,
      true,
    ]);
  ...
  it("uses effective duration for metadata and seek bounds, then clears the bound", () => {
    ...
    vi.advanceTimersByTime(250);
    expect(emissions).toEqual([
      expect.objectContaining({ Metadata: iface.Metadata }),
    ]);
  });
});
...
describe("MPRIS seek bounds", () => {
  ...
  it.each([0n, 10_000_000n])(
    "accepts SetPosition at boundary %s",
    (position) => {
      const iface = initPlayerInterface();
      player.emitNowPlaying({ trackId: "track-1", durationInMillis: 10_000 });
      ...
      expect(winContents.send).toHaveBeenCalledExactlyOnceWith(
        "player:seek",
        Number(position) / 1_000_000,
      );
    },
  );

  it.each([-1n, 10_000_001n, 9_223_372_036_854_775_807n])(
    "ignores invalid SetPosition %s",
    (position) => {
      const iface = initPlayerInterface();
      player.emitNowPlaying({ trackId: "track-1", durationInMillis: 10_000 });
  ...
  it.each([
    ...
    expect(log.scope("mpris").info).toHaveBeenCalledWith(
      `source=mpris method=Seek channel=${channel} result=sent`,
    );
  });
...
describe("MPRIS fixed Rate", () => {
  it("exports writable Rate and read-only limits at normal speed", () => {
    ...
    expect(iface.$introspect().property).toEqual(
      expect.arrayContaining([
    ...
    expect([iface.Rate, iface.MinimumRate, iface.MaximumRate]).toEqual([
      1, 1, 1,
    ]);
  });

  it.each([1, 0.5, 2, -1, NaN, Infinity])(
    "keeps normal speed after setting %s",
    (rate) => {
      vi.useFakeTimers();
      const iface = initPlayerInterface();
      expect(() => {
  ...
  it("pauses when Rate is set to zero without changing the speed", () => {
    ...
    expect(iface.Rate).toBe(1);
    expect(log.scope("mpris").info).toHaveBeenCalledWith(
      "source=mpris method=Rate channel=player:pause result=sent",
    );
  });
});
...
describe("MPRIS OpenUri", () => {
  ...
  it("switches service first and waits for hook readiness rather than load completion", () => {
    ...
    winContents.emit("did-start-navigation", {
      isMainFrame: true,
    ...
    expect(vi.mocked(switchService).mock.invocationCallOrder[0]).toBeLessThan(
      winContents.send.mock.invocationCallOrder[0],
    );
  });
  ...
  it.each([false, true])(
    "cancels pending media on unrelated navigation with sameDocument=%s",
    (isSameDocument) => {
      ...
      winContents.emit("did-start-navigation", {
        isMainFrame: true,
        isSameDocument,
        url: "https://classical.music.apple.com/search",
      });
  ...
  ])("rejects a URI outside the service origin %#", (uri) => {
  ...
  it("advertises HTTPS URI support", () => {
    initPlayerInterface();
    const rootIface = busStub.export.mock.calls[0][1] as {
  ...
  it("loads a music.apple.com URI", () => {
    ...
    expect(win.loadURL).toHaveBeenCalledWith(
      "https://music.apple.com/gb/album/foo",
    );
    expect(log.scope("mpris").info).toHaveBeenCalledWith(
      "source=mpris method=OpenUri result=sent",
    );
  });

  it("loads a classical.music.apple.com URI", () => {
    initPlayerInterface().OpenUri(
      "https://classical.music.apple.com/gb/album/foo",
    );
    expect(win.loadURL).toHaveBeenCalledWith(
      "https://classical.music.apple.com/gb/album/foo",
    );
  ...
  it("does not log accepted or rejected URI request data", () => {
    const iface = initPlayerInterface();
    const acceptedUri =
      "https://music.apple.com/gb/album/private?token=accepted-secret";
  ...
  it("logs an accepted URI as dropped when no window exists", () => {
    initInterfaces(() => null).player.OpenUri(
      "https://music.apple.com/gb/album/foo",
    );
...
describe("MPRIS PlaybackStatus mapping", () => {
  ...
  // A state added to src/player.ts must be added to STATUS_TABLE, or it silently falls through to 'Stopped'.
  it("covers every declared PlaybackState", () => {
    expect(STATUS_TABLE.map(([state]) => state).sort()).toEqual(
      Object.values(PlaybackState).sort(),
    );
  });
...
describe("MPRIS metadata", () => {
  ...
  function flatten(
    metadata: Record<string, VariantValue>,
  ): Record<string, [string, unknown]> {
    return Object.fromEntries(
      Object.entries(metadata).map(([key, variant]) => [
        key,
  ...
  it("builds every field of a full payload with the right signatures", () => {
    ...
    expect(flatten(iface.Metadata)).toEqual({
      ...
      "xesam:url": [
        "s",
        "https://music.apple.com/gb/album/blue-monday/1440857781",
      ],
  ...
  it("clears stale playback data when the document is replaced", () => {
    ...
    expect(emissions.every((properties) => !("Position" in properties))).toBe(
      true,
    );
    expect({
      LoopStatus: iface.LoopStatus,
...
describe("MPRIS radio song metadata", () => {
  const station: NowPlayingPayload = {
    trackId: "ra.123",
    name: "Station",
    artistName: "Station host",
    albumName: "Station album",
    artworkUrl: "https://example.com/station.jpg",
    url: "https://music.apple.com/gb/station/example/ra.123",
  ...
  it.each([null, 7_200_000_000])(
    "updates song details without changing stream state for duration %s",
    (durationUs) => {
      const iface = initPlayerInterface();
      ...
      player.handlePlaybackCapabilitiesDidChange({
        canPlay: true,
        canPause: true,
        canSeek: durationUs === null ? null : true,
        durationUs,
      });
      ...
      expect(iface.Metadata).toMatchObject({
        ...
        "xesam:url": {
          signature: "s",
          value: "https://music.apple.com/song/123",
        },
  ...
  it("publishes catalogue enrichment even when the song title and artist stay unchanged", () => {
    ...
    player.emitTimedMetadata(song);
    vi.advanceTimersByTime(250);
    expect(iface.Metadata["xesam:url"].value).toBe(
  ...
  it("applies downloaded station artwork without restoring stale song metadata", async () => {
    ...
    expect(iface.Metadata["mpris:artUrl"].value).toBe(
      "file:///tmp/station.jpg",
    );
  ...
  it("ignores old artwork after returning to the same station ID", async () => {
    ...
    player.emitNowPlaying({
      ...station,
      artworkUrl: "https://example.com/new-station.jpg",
    });
    player.emitTimedMetadata(song);
    resolveArtwork("/tmp/old-station.jpg");
    await Promise.resolve();
    expect(iface.Metadata["mpris:artUrl"].value).toBe(
      "https://example.com/new-station.jpg",
    );
...
describe("MPRIS xesam:url fallback", () => {
  it("rebuilds the song URL from catalogId when the payload carries no url", () => {
    ...
    player.emitNowPlaying({
      trackId: "999",
      playParams: { catalogId: "999", isLibrary: true, kind: "song" },
    });
  ...
  it("rebuilds the song URL from globalId when catalogId is absent too", () => {
    ...
    player.emitNowPlaying({
      trackId: "abc",
      playParams: { globalId: "abc", kind: "song" },
    });
