...
function createReadableBody(
  data: Buffer = Buffer.from("image-data"),
...
function mockSuccessfulFetch() {
  vi.mocked(net.fetch).mockImplementation(() =>
    Promise.resolve(createMockResponse(200, true, createReadableBody())),
  );
}
...
describe("downloadArtwork", () => {
  ...
  beforeEach(async () => {
    ...
    // Reapply the file-system mocks after resetModules so the fresh import receives them.
    ...
    // Re-import the mocked modules so local references use the same instances.
  ...
  it("resolves with filepath on successful download", async () => {
    ...
    vi.mocked(net.fetch).mockResolvedValue(
      createMockResponse(
        200,
        true,
        createReadableBody(Buffer.from("image-data")),
      ),
    );
    ...
    const writeCall = vi.mocked(fs.createWriteStream).mock
      .calls[0][0] as string;
  ...
  it("fetches once for three concurrent calls with the same URL", async () => {
    ...
    // Three callers share one promise, so they open one temporary file and one stream.
  ...
  it("settles every concurrent caller with null on failure and refetches on a later call", async () => {
    ...
    // A failed promise leaves the cache, so a later call downloads again.
    mockWriteStream();
    mockSuccessfulFetch();
  ...
  it("extracts UUID and size from Apple CDN URL for cache filename", async () => {
    const url =
      "https://is1-ssl.mzstatic.com/image/thumb/Music125/v4/69/4d/b4/694db440-1fdd-0112-16a0-ae501501cb32/14UMGIM07610.rgb.jpg/512x512bb.jpg";

    vi.mocked(fs.existsSync).mockReturnValue(true);

    const result = await downloadArtwork(url);
    expect(result).toMatch(
      /694db440-1fdd-0112-16a0-ae501501cb32-512x512\.jpg$/,
    );
  });
