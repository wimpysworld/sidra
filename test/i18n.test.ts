...
// The records load at module scope, so a read failure needs a fresh import with fs stubbed for that module graph.
// A file-level fs mock would break the localisation tests that read the real files.
...
describe("locale file loading", () => {
  ...
  it("names the file and the asarUnpack rule when the read fails", async () => {
    ...
    await expect(
      ...
      /Test Player could not load the locale file .*loading\.json.*ENOENT[\s\S]*asarUnpack/,
    );
  ...
  it("names the file when the JSON is malformed", async () => {
    await expect(
      importI18nWithRead(() => '{ "LOADING_TEXT": '),
    ).rejects.toThrow(/loading\.json/);
  });
});
