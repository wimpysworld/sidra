...
let bus: EventEmitter & {
  call: ReturnType<
    typeof vi.fn<(message: Message) => Promise<{ body: unknown[] }>>
  >;
...
const track = () => ({
  ...
  previous: "Zurück",
  next: "Weiter",
  playbackAction: "play" as const,
  playbackLabel: "Abspielen",
  onAction,
});
const current = () => true;
...
function notifyCalls(): Message[] {
  return bus.call.mock.calls
    .map(([message]) => message as Message)
    .filter((message) => message.member === "Notify");
}
...
beforeEach(() => {
  ...
  bus = Object.assign(new EventEmitter(), {
    call: vi.fn(async (message: Message) => {
      ...
      if (message.member === "Notify")
        return { body: [message.body[1] || nextId++] };
      return { body: [] };
    }),
    disconnect: vi.fn(),
  ...
  vi.mocked(sessionBus).mockReturnValue(
    bus as unknown as ReturnType<typeof sessionBus>,
  );
...
describe("Linux track notifications", () => {
  ...
  it("encodes artwork paths with spaces, non-ASCII characters and URI delimiters", async () => {
    await adapter.show(
      { ...track(), icon: "/tmp/Album art/Björk #1%.jpg" },
      current,
    );
    expect(notifyCalls()[0].body[6]["image-path"].value).toBe(
      "file:///tmp/Album%20art/Bj%C3%B6rk%20%231%25.jpg",
    );
  });
  ...
  it("refreshes a playback button only on an existing notification with actions", async () => {
    await adapter.show(track(), current);
    await adapter.show(
      { ...track(), playbackAction: "pause", playbackLabel: "Pause" },
      current,
      true,
    );
    expect(notifyCalls()[1].body[1]).toBe(1);
    expect(notifyCalls()[1].body[5]).toEqual([
      "default",
      "",
      "pause",
      "Pause",
  ...
  it.each([true, false])(
    "replaces the previous id when actions are supported: %s",
    async (actions) => {
  ...
  it.each([false, true])(
    "releases a hung Notify on daemon replacement, after timeout: %s",
    async (expire) => {
      let resolveNotify!: (reply: { body: unknown[] }) => void;
      const oldAction = vi.fn();
      const original = bus.call.getMockImplementation()!;
      ...
      vi.useFakeTimers();
      try {
        const first = adapter.show(
          { ...track(), onAction: oldAction },
          current,
        );
  ...
  it("closes the eager shared connection on quit before playback", async () => {
    await vi.waitFor(() => {
      expect(
        bus.call.mock.calls.some(
          ([message]) => message.member === "GetNameOwner",
        ),
      ).toBe(true);
    });
  ...
  it("detaches listeners and destroys the socket on quit", async () => {
    ...
    expect(bus._connection.stream.destroy).toHaveBeenCalledOnce();
    const close = bus.call.mock.calls
      .map(([message]) => message)
      .filter((message) => message.member === "CloseNotification");
  ...
  it("bounds cleanup when CloseNotification never replies", async () => {
    ...
    const original = bus.call.getMockImplementation()!;
    bus.call.mockImplementation((message: Message) =>
      message.member === "CloseNotification"
        ? new Promise(() => {})
        : original(message),
    );
    vi.useFakeTimers();
  ...
  it("releases the socket when CloseNotification fails", async () => {
    ...
    bus.call.mockImplementation((message: Message) =>
      message.member === "CloseNotification"
        ? Promise.reject(new Error("No owner"))
        : original(message),
    );
  ...
  it("handles a session bus that cannot be opened", async () => {
    adapter.dispose();
    vi.mocked(sessionBus).mockImplementationOnce(() => {
