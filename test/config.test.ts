...
describe("Config store type assertions", () => {
  ...
  it("getLanguage returns string | null | undefined", () => {
    expectTypeOf(getLanguage).returns.toEqualTypeOf<
  ...
  it("getStartPage returns start page union", () => {
    expectTypeOf(getStartPage).returns.toEqualTypeOf<
      "home" | "new" | "radio" | "all-playlists" | "last"
    >();
  });

  it("setStartPage accepts start page union", () => {
    expectTypeOf(setStartPage)
      .parameter(0)
      .toEqualTypeOf<"home" | "new" | "radio" | "all-playlists" | "last">();
  });
  ...
  it("getClassicalStartPage returns the Classical start page union", () => {
    expectTypeOf(getClassicalStartPage).returns.toEqualTypeOf<
      ClassicalStartPageId | "last"
    >();
  });

  it("setClassicalStartPage accepts the Classical start page union", () => {
    expectTypeOf(setClassicalStartPage)
      .parameter(0)
      .toEqualTypeOf<ClassicalStartPageId | "last">();
  });

  it("getClassicalLastPageUrl returns string | undefined", () => {
    expectTypeOf(getClassicalLastPageUrl).returns.toEqualTypeOf<
      string | undefined
    >();
  });
  ...
  it("getStartPageFor takes a MusicServiceId and returns the wide start page union", () => {
    ...
    expectTypeOf(getStartPageFor).returns.toEqualTypeOf<
      AnyStartPageId | "last"
    >();
  });

  it("getLastPageUrlFor takes a MusicServiceId and returns string | undefined", () => {
    expectTypeOf(getLastPageUrlFor)
      .parameter(0)
  ...
  it("setLastPageUrlFor takes a MusicServiceId and a string", () => {
    expectTypeOf(setLastPageUrlFor)
      .parameter(0)
      .toEqualTypeOf<MusicServiceId>();
...
describe("Config store runtime behaviour", () => {
  // The electron-conf mock exposes its one process-wide backing map for fixture seeding.
  ...
  it("each last page reader reads its own service key", () => {
    ...
    expect(getClassicalLastPageUrl()).toBe(
      "https://classical.music.apple.com/gb/browse/catalog",
    );
  });
  ...
  // SERVICE_PAGE_ACCESSORS is the only map from a service id to stored keys.
  // Each getStartPageFor assertion covers both ids, so swapped rows cannot pass.
